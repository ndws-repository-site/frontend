...
/**
 * Выпадающий список админки. Варианты: standard, alternative.
 * Опционально: поиск (isSearchable), загрузка (isLoading), очистка значения.
 */
export const AdminSelect = ({
    ...
    const filteredOptions = useMemo(() => {
        ...
        return options.filter((option) => {
            ...
            if (option.keywords && option.keywords.includes(lowerQuery))
                return true;

            // 2. Поиск по значению (value)
            if (String(option.value).toLowerCase().includes(lowerQuery))
                return true;
