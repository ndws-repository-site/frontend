...
/**
 * Карточка профиля в боковой панели: аватар, имя, роль (админ/модератор). Состояние загрузки — скелетон.
 */
export const ProfileCard = ({
    ...
    if (loading) {
        return (
            <div
                className={cn(
                    "bg-white/8 rounded-[20px] p-0.5 pr-4 flex items-center gap-2 animate-pulse select-none",
                    className,
                )}
            >
    ...
    return (
        <div
            className={cn(
                "bg-white/8 rounded-[20px] p-0.5 pr-4 flex items-center gap-2",
                className,
            )}
        >
            <Image
